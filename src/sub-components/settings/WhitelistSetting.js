...
const WhitelistSetting = ({ whitelist }) => {
    ...
    console.log("exception data from general settings:", whitelist);
    ...
    useEffect(() => {
        if (whitelist) {
            setFormData({
                domainName: whitelist.domainName || '',
                status: whitelist.status || '',
                createdDate: whitelist.createdDate || '',
                updatedDate: whitelist.updatedDate || ''
            });
        }
    }, [whitelist]);
